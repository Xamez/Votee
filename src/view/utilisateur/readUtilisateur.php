<div class="flex flex-col md:flex-row justify-center gap-6 md:gap-16">
    <div class="flex flex-col">
        <div class="flex flex-col md:flex-row gap-4">
            <div class="flex flex-col">
                <p for="prenom" class="text-main text-2xl font-bold">Prénom</p>
                <input readonly type="text" value="<?= $utilisateur->getPrenom();?>" name="prenom" id="prenom">
            </div>
            <div class="flex flex-col">
                <label for="nom" class="text-main text-2xl font-bold">Nom</label>
                <input readonly type="text" value="<?= $utilisateur->getNom();?>" name="nom" id="nom">
            </div>
        </div>
        <div class="flex flex-col">
            <label for="description" class="text-main text-xl">Description</label>
            <textarea readonly name="description" id="description" rows="10" class="border-2 border-gray-200 rounded-md max-h-40 resize-none"><?= $utilisateur->getDescription();?></textarea>
        </div>
        <div class="flex flex-wrap flex-row-reverse gap-2">
            ...
            foreach ($groupes as $groupe) {
                echo '<a href="./frontController.php?controller=groupe&action=readGroupe&idGroupe=' . rawurlencode($groupe->getIdGroupe()) . '">
                            <div class="bg-white flex gap-1 text-main shadow-md rounded-xl w-fit p-2">
    ...
    <?php
    AbstractController::afficheVue('button.php', ['controller' => 'utilisateur', 'action' => 'updateUtilisateur', 'title' => 'Éditer mon profile', "logo" => 'edit']);
    ?>
    </div>
</div>
