<h1 class="title text-dark text-2xl font-semibold">Éditer son profil</h1>

<form class="flex flex-col justify-center gap-8" method="post" action="frontController.php?controller=utilisateur&action=confirmUpdateUtilisateur">
    <div class="flex flex-col md:flex-row justify-center gap-6 md:gap-16">
        <div class="flex flex-col">
            <div class="flex flex-col md:flex-row gap-4">
                <div class="flex flex-col">
                    <label for="prenom" class="text-main text-2xl font-bold">Prénom</label>
                    <input required type="text" value="<?= $utilisateur->getPrenom();?>" name="prenom" id="prenom">
                </div>
                <div class="flex flex-col">
                    <label for="nom" class="text-main text-2xl font-bold">Nom</label>
                    <input required type="text" value="<?= $utilisateur->getNom();?>" name="nom" id="nom">
                </div>
            </div>
            <div class="flex flex-col">
                <label for="description" class="text-main text-xl">Description</label>
                <textarea required name="description" id="description" rows="10" class="border-2 border-gray-200 rounded-md max-h-40 resize-none"><?= $utilisateur->getDescription();?></textarea>
            </div>
