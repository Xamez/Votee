...
echo '<div class="flex items-center flex-wrap gap-2 pt-0"><p class="text-main font-semibold">Co-auteur :</p>
        <div class="flex gap-1 text-main bg-white shadow-md rounded-2xl p-2"><span class="material-symbols-outlined">account_circle</span>'
    . htmlspecialchars($coAuteur->getNom()) . ' ' . htmlspecialchars($coAuteur->getPrenom()) . '</div></div>';
...
echo '</div><div class="flex gap-2 justify-end">
    <a class="w-36 flex p-2 justify-center text-white bg-dark font-semibold rounded-lg" href="./frontController.php?action=deletePropsition&idProposition=' . $section->getIdProposition() . '">Supprimer</a>
